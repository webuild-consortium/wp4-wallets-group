        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 w-full flex-1 overflow-hidden flex flex-col relative min-h-[500px]">
            <div className="bg-slate-50 border-b border-gray-200 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 z-20 relative">
                <div className="flex items-center gap-4">
                    <ProviderIdentity id={entry.id} shortName={entry.shortName} legalName={entry.legalName} size="lg" />
                    <button 
            ...
            <div className="relative flex-1 bg-white">
                ...
                <div className={`p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 ${!entry.hasResponse ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
                    ...
                    <div className="space-y-4">
                        ...
                        <div><span className="text-xs text-gray-500 mb-2 block">Offline Channels:</span><div className="flex flex-wrap gap-2">{entry.offlineChannels.length ? entry.offlineChannels.map(c => <StandardChip key={c} label={c} variant="purple" />) : <span className="text-sm text-gray-400 italic">None</span>}</div></div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            {[
                                { label: 'Protocols', items: entry.protocols, variant: 'indigo' as const },
                                { label: 'Encodings', items: entry.encodings, variant: 'amber' as const },
                                { label: 'Disclosure', items: entry.disclosure, variant: 'rose' as const }
                            ].map(g => (
                                <div key={g.label}><span className="text-xs text-gray-500 mb-1 block">{g.label}:</span><div className="flex flex-wrap gap-1">{g.items.length ? g.items.map(i => <StandardChip key={i} label={i} variant={g.variant} />) : <span className="text-sm text-gray-400 italic">None</span>}</div></div>
                            ))}
                        </div>
                    </div>
